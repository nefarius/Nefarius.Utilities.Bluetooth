...
public static class SdpPatcher
{
    ...
    public static bool AlterHidDeviceToVendorDefined(byte[] input, out byte[] output)
    {
        try
        {
            ...
            var sizeFieldsOffsets = new BoyerMoore([0x36])
            ...
            parser.GlobalItemParsed += item =>
            {
                ...
                if (!item.IsUsagePage || item.ItemSize != 1) return;
            ...
            foreach (var offset in sizeFieldsOffsets)
            {
                ...
                patchedRecord.AddRange(BitConverter.GetBytes((ushort)newSize).AsEnumerable().Reverse());
            ...
            patchedRecord.AddRange(BitConverter.GetBytes((ushort)patchedHidReportDescriptor.Count).AsEnumerable()
                .Reverse());
    ...
    private static bool TryFallbackMethod(byte[] input, out byte[] output)
    {
        try
        {
            ...
            var sizeElements = sizeElementsIndexes
            ...
            var descriptorStartIndex = new BoyerMoore(descriptorStartPattern).Search(input).First();

            // get descriptor length
            var descriptorSizeIndex = descriptorStartIndex - 1;
            var descriptorSize = input[descriptorSizeIndex];
            var descriptorEnd = descriptorStartIndex + descriptorSize;

            // get original descriptor
            var hidReportDescriptor = input.Skip(descriptorStartIndex).Take(descriptorSize).ToList();
            ...
            var patchedDescriptorSize = patchedHidReportDescriptor.Count;

            // need to add the growth difference to list size elements
            var sizeDifference = patchedDescriptorSize - descriptorSize;
